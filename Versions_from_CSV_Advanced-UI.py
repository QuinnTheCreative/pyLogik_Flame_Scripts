# -*- coding: utf-8 -*-
"""
Script Name: CSV Sequence Duplicator (Flame UI Style)
...
Description:
    ...
    sequences with names from CSV file.
...
class CSVDuplicatorWindow(QtWidgets.QWidget):
    ...
    def load_csv_preview(self):
        ...
        for i, name in enumerate(self.csv_names[:50]):  # Show first 50
            self.csv_preview_list.addItem(f"{i+1:2d}. {name}")
        
        if len(self.csv_names) > 50:
            self.csv_preview_list.addItem(f"... and {len(self.csv_names) - 50} more names")
        
        # Update count
        self.csv_count_label.setText(f"Names found: {len(self.csv_names)}")
    ...
    def duplicate_sequences(self):
        """Duplicate sequences with progress tracking."""
        ...
        self.progress_label.setText(f"Complete! Created {success_count} duplicates, {error_count} errors")
        
        message(f"Duplication complete: {success_count} created, {error_count} errors")
