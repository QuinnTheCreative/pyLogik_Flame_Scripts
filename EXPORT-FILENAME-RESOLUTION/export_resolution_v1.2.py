...
---------------------------------
flame.projects.current_project exposes:
  .name                  — the Flame project name string
  .get_wiretap_storage_id() — the stonefs volume name (e.g. "stonefs_2025")

From those two values the project root on disk is:
  /opt/Autodesk/clip/<storage_id>/<project_name>

The export destination is then:
  <project_root>/06_EXPORTS

If the storage ID cannot be resolved, the script falls back to BROWSE_START_PATH
so the user can still pick a folder manually.
...
def get_project_export_path():
    ...
    Resolution order
    ----------------
    1.  flame.projects.current_project.name
            → the Flame project name (e.g. "MY_PROJECT_flame01")
    2.  flame.projects.current_project.get_wiretap_storage_id()
            → the stonefs volume name (e.g. "stonefs_2025")
        Together these give the project root on disk:
            /opt/Autodesk/clip/<storage_id>/<project_name>
    3.  Append "06_EXPORTS" to that root.

    Returns the derived path string, or None if the project / storage
    information cannot be resolved (caller falls back to BROWSE_START_PATH).
    """
    import flame
    ...
    try:
        ...
        storage_id   = str(project.get_wiretap_storage_id())

        if not project_name or not storage_id:
            return None
        ...
        print(
            "[exportHook] Project: '{}' | Storage: '{}' | Export path: {}".format(
                project_name, storage_id, export_path
            )
        )
        return export_path
