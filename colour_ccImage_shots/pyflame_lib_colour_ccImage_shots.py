...
Version: 2.1.8
Written by: Michael Vaglienty modified by QuinnR
...
class PyFlameFunctions():
    ...
    def get_flame_python_packages_path(print_path: bool=True) -> str:
        ...
        if not isinstance(print_path, bool):
            raise TypeError('Pyflame Get Flame Python Packages Path: print_path must be a boolean.')
    ...
    def untar(
        ...
        if not isinstance(tar_file_path, str):
            raise TypeError('pyflame.untar: tar_file_path must be a string')
    ...
    def get_export_preset_version(preset_path: str) -> Tuple[str, str]:
        ...
        if not isinstance(preset_path, str):
            raise TypeError('Pyflame Get Export Preset Version: preset_path must be a string.')
    ...
    def update_export_preset(preset_path: str) -> None:
        ...
        # Validate argument types
        if not isinstance(preset_path, str):
            raise TypeError('Pyflame Update Export Preset: preset_path must be a string.')
